...
describe("stakeAndLockFarm", function () {
  before(async function () {
    await prepare(this, ['ERC20Mock', 'stakeAndLockFarm'])
  })
  ...
  beforeEach(async function () {
    ...
    await deploy(this, [
        ['farm', this.stakeAndLockFarm, [this.token.address, getBigNumber(1,18)]] // reward = 1 Token per block 
    ])
  ...
  describe("Combining Transactions", function() {
    it("Claim and Withdraw in one batch should not be possible", async function () {
      await this.farm.add(10, this.lps.address)
      await this.lps.approve(this.farm.address, getBigNumber(10))
      ...
      const msg1 = 'stakeAndLockFarm::farm is operating normally. Withdrawals are not permitted.';

      let rewardForAlice = await this.farm.pendingReward(0, this.alice.address)
      await expect(this.farm.batch(
        [
      ...
      await this.farm.setOperational(false);

      const msg2 = 'stakeAndLockFarm::farm is closed. Please withdraw your deposits';
      ...
      await this.farm.setOperational(true);
  ...
  describe("Add", function () {
    ...
    it("Should not allow adding the same LP twice", async function () {
      const msg1 = "stakeAndLockFarm::there is already a pool with this LP";
  ...
  describe("Non Reentrant", function () {
    it("Should block the farm", async function () {
      ...
      const msg2 = 'stakeAndLockFarm::nonReentrant - try again';
  ...
  describe("Withdraw", function () {
    ...
    it("Partial withdraw should not affect pending rewards (with one user)", async function () {
      ...
      await this.farm.setOperational(false);
      await this.farm.withdraw(0, getBigNumber(1,3), this.alice.address)
      let l2 = await this.farm.setOperational(true);
    ...
    it("Partial withdraw should not affect pending rewards (with two users)", async function () {
      ...
      await this.farm.setOperational(true);
      pendingReward = await this.farm.pendingReward(0, this.alice.address)
      let expectedReward = getBigNumber(1,18).mul(bl1.blockNumber - al1.blockNumber).div(2)
        .add(getBigNumber(1,18))
        .add(getBigNumber(1,18).mul(al2.blockNumber - bl1.blockNumber + 2).div(3))
      expect(pendingReward).to.be.equal(expectedReward)
    })
  ...
  describe("EmergencyWithdraw", function() {
    ...
    it("Cannot withdraw to zero address", async function () {
      ...
      const msg1 = "stakeAndLockFarm::can't withdraw to address zero";
